...
class token:
    def __init__(self,type_,valeur,nb_ligne):
        self.type_ = type_
        self.valeur = valeur
        self.nb_line = nb_ligne

    def __str__(self):
        str_ret = ""
        if(self.valeur == None):
            str_ret = "type : "+self.type_
        else:
            str_ret = "type : "+self.type_+" ; valeur : "+str(self.valeur)
        str_ret = str_ret + "; in line : " + str(self.nb_line)
        return str_ret
...
def accept(T):
	global current_token
	if(not check(T)):
		raise Exception("Error Fatal : Expected '"+str(T)+"' ; but found '"+str(current_token.type_)+"' ; in line : "+str(int(current_token.nb_line))+" ")
...
def noeudS():
    ...
    if(check('(')):
        ...
        e = noeudE(0)
        accept(']')
        A = Noeud('+',None,[N,e],current_token.nb_line)
        N = Noeud('indir',None,[A],last_token.nb_line)
...
def compile():
	...
	lib_filename = "/home/ubuntu/Desktop/lib.c" 
	lib_filename = lib_filename.replace('\\','\\\\',len(lib_filename))
    
	#code_filename=input("Enter the name of the code file with the full path   : ")
	code_filename = "/home/ubuntu/Desktop/code.c"
	code_filename = code_filename.replace('\\','\\\\',len(code_filename))
	
	#out_filename = input("Do you need the output to be in a file rather than the terminal  Y: yes      N: No")
	out_filename = "y";
	if(out_filename.lower()=='y'):
		#out_filename = input("Enter the name of the output file with the full path   : ")
		out_filename = "/home/ubuntu/Desktop/ass.txt"
		out_filename = out_filename.replace('\\','\\\\',len(out_filename))
